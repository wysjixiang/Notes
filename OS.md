...
8. #### 访问方法-文件存储信息，当使用时，需要访问信息并将其读取到计算机内存
   ...
   1. 顺序访问：文件信息按顺序加以处理，顺序就是一个记录接着一个记录。这种访问模式是最常见的，如编辑器，编译器。
   2. 直接访问：也叫相对访问。这里文件由固定长度的逻辑记录组成，按人一顺序进行快速读取和写入记录。也就是可以任意顺序读取数据块，这对于大量信息的访问十分高效，经常在数据库中使用。这里会涉及到文件块号的部分，相当于一个索引，并且这个块号是相对于文件开头的索引，和elf文件的索引也是类似的！
   3. 其他方式：如索引访问方式。这就和elf文件中的索引访问差不多了！通过头表找到目标位置的索引，然后查询相关的表，找到具体的地址，然后访问。

9. #### 目录与磁盘结构

   1. 一个存储设备可以按整体来用于文件系统，也可以分区，每个分区有单独的文件系统。
   2. 包含文件系统的分区称为卷。卷可以是设备的
   3. 目录可以视为符号表，将文件名称转换成目录条目
   4. 目录结构有多种，如单级目录、两级目录（需要路径名的搭配）、树形目录
   5. 可以使用无环图组织文件目录，使得文件得到共享
   6. 共享文件和目录的实现，常用的一种方式是创建一个名为链接的新目录条目。这是UNIX系统所采用的。链接实际上是另一个文件或子目录的指针。
   7. 软链接和硬链接：软链接相当于把文件的绝对路径给拷贝过去了。硬链接：文件在分区中会有一个索引节点号 Inode Index。硬链接会创建一个文件名并链接这个节点号。即存在多个不同的文件名指向相同的节点号，这样能避免误删除。只有当最后一个指向索引节点号的文件被删除了，这个文件才会真正的从分区设备中删除
...
1. I/O设备的基本要素，如端口、总线及设备控制器，适用于各种各样的I/O设备。为了封装各种设备细节与特点，操作系统内核采用设备驱动程序模块。设备驱动程序为I/O子系统提供了统一的设备访问接口

2. 菊花链：如果设备A通过线路连接到设备B，B又通过线路连接到设备C，C再通过端口连接到计算机，这种方式成为菊花链。

3. 内存映射(MMIO)：首先，MMIO功能需要设备控制器的支持，内存和I/O设备共享同一个地址空间

4. I/O端口通常由四个寄存器组成，即状态、控制、数据输入和数据输出寄存器

5. #### 中断

   1. 对于现代操作系统，需要复杂的中断处理功能。
   2. 在关键处理时，需要能够延迟中断处理；需要有效的方式分派中断到合适的中断处理程序，而无需依次轮询查找；需要多级中断机制。这3个功能可由CPU和中断控制器来提供
   3. 大多数CPU有两条中断请求线，一条是非屏蔽中断，诸如不可恢复的内存错误等事件；一条是可屏蔽中断，在执行不得中断的关键指令序列之前，它可以由CPU关闭

6. #### 直接内存访问DMA:

   1. DMA控制器与设备控制器之间的握手，通过一对称为DMA请求和DMA确认的线路来进行。当有数据需要传输时，设备控制器发送信号到DMA请求线路。这个信号使得DMA控制器占用内存总线，然后发送确认信号给设备控制器，开始发送数据。发送完成之后，产生DMA中断。 
   2. 同时，DMA也支持直接虚拟内存访问，即DVMA

7. UNIX系统有一个后门，以便用户程序透明传递任何命令到设备控制器，即系统调用ioctl()。                                                                                                      
...
**<u>父、子进程共享全部数据，但并不是说他们就是对同一块数据进行操作，子进程在读写数据时会通过写时复制机制将公共的数据重新拷贝一份，之后在拷贝出的数据上进行操作</u>**。
...
1. 进程创建与结束
   ...
   - 背景知识：
     ...
     <u>**进程有两种创建方式，一种是操作系统创建的一种是父进程创建的。从计算机启动到终端执行程序的过程为：0号进程 -> 1号内核进程 -> 1号用户进程(init进程) -> getty进程 -> shell进程 -> 命令行执行进程。所以我们在命令行中通过 ./program执行可执行文件时，所有创建的进程都是shell进程的子进程，这也就是为什么shell一关闭，在shell中执行的进程都自动被关闭的原因。从shell进程到创建其他子进程需要通过以下接口。**</u>
...
- 进程地址空间（地址空间）
  ...
  有一些"敏感"的地址需要注意下，对于32位进程来说，代码段从0x08048000开始。从0xC0000000开始到0xFFFFFFFF是内核地址空间，通常情况下代码运行在用户态（使用0x00000000 ~ 0xC00000000的用户地址空间），当发生系统调用、进程切换等操作时CPU控制寄存器设置模式位，进入内核模式，在该状态（超级用户模式）下进程可以访问全部存储器位置和执行全部指令。(这段说的有点问题。进入内核模式并不是说你的指令运行到内核所在的空间就是内核模式了。尽管确实是如此，但是并不能这样说明～而应该通过系统调用、异常这些概念来说明，只有进入内核模式才能访问到该权限模式下的某些数据，譬如CPU内部寄存器、外设地址等等重要的、不该暴露给用户的数据)
...
- 上下文切换
  ...
  内核管理所有进程控制块，而进程控制块记录了进程全部状态信息。每一次进程调度就是一次上下文切换，所谓的上下文本质上就是当前运行状态，主要包括通用寄存器、浮点寄存器、状态寄存器、程序计数器、用户栈和<u>***内核数据结构（页表、进程表、文件表）***</u>等。
...
## 9、如果系统中具有快表后，那么地址的转换过程变成什么样了？这里的快表指的就是TLB(Translation Look-aside Table)
...
| ---------------------- | ------------------------------------------------------------ | ----------------------------------------------- |
| 基本地址变换机构       | ①算页号、页内偏移量 ②检查页号合法性 ③查页表，找到页面存放的内存块号 ④根据内存块号与页内偏移量得到物理地址 ⑤访问目标内存单元 | 两次访存(这可不一定是2次吧)                     |
...
<u>***从操作系统层面上看，malloc是通过两个系统调用来实现的： brk和mmap***</u>
...
**软件中断与软中断**
***<u>除了硬件中断，还存在部分由软件产生的中断，称之为软件中断（Software Interrupt），最常见的有引发系统调用的Int 0x80。同时，软件中断又区别于软中断（SoftIRQ），软中断主要用于中断处理的下半程（Bottom Halves），非关键逻辑的部分，来提高中断处理的效率与实时性，最常用于I/O相关的中断处理。***</u>
...
**<u>*刚刚换出的页面马上又要换入内存，刚刚换入的页面马上又要换出外存，这种频繁的页面调度行为称为抖动，或颠簸。产生抖动的主要原因是进程频繁访问的页面数目高于可用的物理块数(分配给进程的物理块不够)*</u>**
...
**<u>*可优先换出阻塞进程;可换出优先级低的进程;为了防止优先级低的进程在被调入内存后很快又被换出，有的系统还会考虑进程在内存的驻留时间…*</u>**
...
**<u>*并发是指宏观上在一段时间内能同时运行多个程序，而并行则指同一时刻能运行多个指令*</u>**。
...
![](http://oss.interviewguide.cn/img/202205212345108.png)
...
![](http://oss.interviewguide.cn/img/202205212345413.png)

![](http://oss.interviewguide.cn/img/202205212345552.png)Belady异常—当为进程分配的物理块数增大时，缺页次数不减反增的异常现象。
...
![](http://oss.interviewguide.cn/img/202205212345619.png)
...
![](http://oss.interviewguide.cn/img/202205212345526.png)
...
![](http://oss.interviewguide.cn/img/202205212345046.png)
...
***<u>把头埋在沙子里，假装根本没发生问题。</u>***

***<u>因为解决死锁问题的代价很高，因此鸵鸟策略这种不采取任务措施的方案会获得更高的性能。</u>***
...
![](http://oss.interviewguide.cn/img/202205212345923.png)
...
![](http://oss.interviewguide.cn/img/202205212345716.png)

上图 c 为不安全状态，因此算法会拒绝之前的请求，从而避免进入图 c 中的状态。

3. #### **多个资源的银行家算法**

![](http://oss.interviewguide.cn/img/202205212345258.png)
